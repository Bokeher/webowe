...
if (liczba >= 1 && liczba <= 12) {
    ...
    console.log(`Liczba ${liczba} nie moze byc numerem miesiaca`);
//zad2
const liczba2 = 12;
if (liczba2 % 2 == 0)
    console.log(`Liczba ${liczba2} jest liczba parzysta`);
else
    console.log(`Liczba ${liczba2} jest liczba nieparzysta`);
//zad3
const dzien = 25;
const miesiac = 1;
const rok = 1988;
const date = new Date();
const dzisiejszyDzien = date.getDate();
const dzisiejszyMiesiac = date.getMonth() + 1;
const dzisiejszyRok = date.getFullYear();
const polskieObywatelstwo = true;
if (polskieObywatelstwo)
    if (rok < dzisiejszyRok - 35 || ((rok == (dzisiejszyRok - 35) && dzien <= dzisiejszyDzien && miesiac <= dzisiejszyMiesiac))) {
        ...
        console.log(`Nie mozesz sie ubiegac o fotel prezydenta`);
...
function wylicz(dlugoscTrasy) {
    if (dlugoscTrasy <= 15)
        return 3;
    if (dlugoscTrasy <= 40)
        return 1.5 + dlugoscTrasy * 0.2;
